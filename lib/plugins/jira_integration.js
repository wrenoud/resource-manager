...
module.exports = function(interface){
  ...
  var serializeUser = function(user, done) {
    return done(null, {key: user.key, password: user.password});
  };
  ...
  interface.registerAuthentication(PLUGIN_NAME, strategy, serializeUser, deserializeUser,
    { 
      successRedirect: '/',
      failureRedirect: '/'
    }
  ...
  var apiRoute = interface.registerAPI(PLUGIN_NAME, router);

  interface.registerUserPicker(PLUGIN_NAME, apiRoute + '/users');
  interface.registerGroupPicker(PLUGIN_NAME, apiRoute + '/groups');
