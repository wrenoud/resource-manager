...
pluginInterface.prototype.initializeAuthentications = function(name, strategy){
  ...
  passport.serializeUser(function(user, done) {
    if(user.strategy in this.authentications){
      this.authentications.serializeUser(user, function(serializedUser){
        done({strategy: user.strategy, data: serializedUser})
      })
    }
  });
  passport.deserializeUser(function(serializedUser, done) {
    if(serializedUser.strategy in this.authentications){
      this.authentications.deserializeUser(serializedUser.data, done);
    }
  });
